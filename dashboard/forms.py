...
class AddCurrencyWizard(forms.Form):
    ...
    starts_at = forms.DateTimeField(label = 'Start Date', required = False, help_text = "Enter date as YYYY-MM-DD", widget = DatePickerInput(format='%m/%d/%Y'))
    ends_at = forms.DateTimeField(label = 'End Date', required = False)

    points_expire = forms.CharField(label = 'Points expire after', required = False, help_text = "Enter time(in days) that points will expire after", widget = DatePickerInput(format='%m/%d/%Y'))
